	...
	<div class="content">
		<form action="<c:url value="/spider/link/save"/>" method="post" role="form" class="form-horizontal">
			...
			<fieldset>
				...
				<div class="form-group">
					<label class="col-md-2 control-label" for="isCrawled"> Site </label>
					<div class="col-md-10">
				...
				<div class="form-group">
					<label class="col-md-2 control-label" for="lastCrawlDate"> <fmt:message key="spider.link.lastCrawlDate"></fmt:message>
					</label>
					<div class="col-md-10">
						<select name="type" class="form-control">
							<option>Select Link Type</option>
							<option value="0" <c:if test="${link.type eq 0 }">selected</c:if>>Seed Page</option>
							<option value="1" <c:if test="${link.type eq 1 }">selected</c:if>>List Page</option>
							<option value="2" <c:if test="${link.type eq 2 }">selected</c:if>>Detail Page</option>
						</select>
					</div>
				</div>
