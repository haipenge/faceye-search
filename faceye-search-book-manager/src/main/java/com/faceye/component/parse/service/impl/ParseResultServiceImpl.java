...
public class ParseResultServiceImpl extends BaseMongoServiceImpl<ParseResult, Long, ParseResultRepository> implements ParseResultService {
	...
	private boolean saveParseResult2Mongo(ParseResult parseResult, Boolean isIgnoreFilterWord) {
		...
		if (StringUtils.isEmpty(content) || StringUtils.isEmpty(name) || content.length() < 100 || name.length() < 5) {
	...
	public Page<ParseResult> getPage(Map<String, Object> searchParams, int page, int size) throws ServiceException {
		...
		if (MapUtils.isNotEmpty(searchParams)) {
			if (StringUtils.equals(MapUtils.getString(searchParams, "isPush2Mongo", "0"), "0")) {
			...
			if (StringUtils.isNotEmpty(MapUtils.getString(searchParams, "categoryId"))) {
				builder.and(qParseResult.categoryId.eq(MapUtils.getLong(searchParams, "categoryId")));
			}
			// 根据级别查询
			if (StringUtils.isNotEmpty(MapUtils.getString(searchParams, "level"))) {
		...
		if (predicate != null) {
			logger.debug(">>FaceYe query parseResult params is:" + predicate.toString());
		}
		Sort sort = new Sort(Direction.DESC, "createDate");
		Pageable pageable = new PageRequest(page, size, sort);
		Page<ParseResult> res = this.dao.findAll(predicate, pageable);
		if (predicate != null) {
			logger.debug(">>FaceYe query parseResult params is:" + predicate.toString()+",page is:"+page+",size is:"+size+",query result size is:"+res.getContent().size());
		}
		return res;
	}
	...
	public void saveAuthPublish() {
		...
		searchParams.put("isPush2Mongo", "0");
		Page<ParseResult> page = this.getPage(searchParams, 1, 5);
