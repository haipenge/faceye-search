...
<div class="col-md-7 col-md-offset-1 bg-white">
	...
	<div class="page-header">
		<h3>${category.name}</h3>
		<div class="content">${category.remark}</div>
	</div>
	<div class="content">
...
<div class="col-md-3 bg-white" style="margin-left:5px;">
	<div class="content">
		<h4>
			<fmt:message key="search.article.hot" />
		</h4>
		<ul class="list-unstyled">
			<c:forEach items="${searchResults.content}" var="searchResult">
				<li><a href="<c:url value="${host}/search/${searchResult.id}.html"/>"><c:if
							test="${fn:length(searchResult.name) > 26}">
							<c:out value="${fn:substring(searchResult.name, 0, 26)}..." />
						</c:if> <c:if test="${fn:length(searchResult.name) <= 26}">${searchResult.name}</c:if></a></li>
			</c:forEach>
		</ul>
	</div>
	<div class="content">
