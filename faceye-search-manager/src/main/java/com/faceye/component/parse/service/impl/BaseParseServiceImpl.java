 ...
 * 基础解析类
 * 
 * @author @haipenge haipenge@gmail.com Create Date:2014年7月8日
 */
abstract public class BaseParseServiceImpl {
	 ...
	 * @todo
	 * @return
	 * @author:@haipenge haipenge@gmail.com 2015年1月19日
	 */
	 ...
	 * @todo
	 * @param crawlResult
	 * @author:@haipenge haipenge@gmail.com 2015年1月1日
	 */
	 ...
	 * @param type
	 * @return
	 * @author:@haipenge haipenge@gmail.com 2015年1月4日
	 */
	 ...
	 * @author:@haipenge haipenge@gmail.com 2015年1月1日
	 */
	protected void saveParseResult(CrawlResult crawlResult, MetaData meta, String title, String content, String categoryName, String categoryAlias) {
		boolean isStore = false;
		title = StringUtils.trim(title);
		isStore = this.isStore(title, content);
		if (isStore) {
			// 是否有重名文章,因已做了URL唯一性验证，存在同名文章的可能性降低，不再进行同名文章检验，放在推送环节检验
		...
		if (isStore) {
			...
			if (siteId != null) {
				...
				if (site != null && site.getIsCrawlImage()) {
					content = this.parseImage(crawlResult, content);
				}
			}
			if (StringUtils.isNotEmpty(content)) {
			...
			if (StringUtils.isNotEmpty(categoryName) && StringUtils.isNotEmpty(categoryAlias)) {
				category = this.articleCategoryService.getArticleCategoryByName(categoryName);
				if (category == null) {
				...
				category = this.buildArticleCategoryInitService.buildArticleCategory(title, content);
			}
			...
			parseResult.setIsWeiXin(crawlResult.getIsWeixin());
	 ...
	 * 设置解析结果的级别
	 * 
	 * 文章 级别 1级：不包含禁词，标题长度大于10个符号，内容长度大于500个字符，归属于具体分类 2级：不包含禁词，标题长度大于10个字符，内容长度在200-500字符，归属于具体分类 3级：不包含禁词，标题长度大于10个字符，内容长度大于500个字符，归属于其它分类 4级：不包含禁词，标题长度小于10个字符，长度不限，有分类或无分类
	 * ------------------------------------------------------------------- 5级:包含禁词,标题长度大于10个符号，内容长度大于500个字符。归属于具体分类 6级,包含禁词，标题长度大于10字符,内容长度在200-500字符，归属于具体分类
	 * 7级,包含禁词，标题长度大于10个字符，内容长度大于500字符，归属于其它分类 8级,包含禁词，标题长度小于10个字符，长度不限。有分类或无分类 ------------------------------------------------------------------- 0级,未归类上以8类级别的解析结果
	 * 
	 * @todo
	 * @param parseResult
	 * @author:@haipenge haipenge@gmail.com 2015年1月28日
	 */
	private void setParseResultLevel(ParseResult parseResult) {
		Integer level = 0;
		int nameLength = StringUtils.length(parseResult.getName());
		int contentLength = StringUtils.length(HtmlUtil.getInstance().replaceAll(parseResult.getContent()));
		// 包含禁词
	...
	/**
	 * 回调方法，由子类实现 增加parseResult将要存储的内容
	 * 
	 * @todo
	 * @param parseResult
	 * @author:@haipenge haipenge@gmail.com 2015年1月1日
	...
	protected boolean isNotHaveSaveNameParseResult(String title) {
		boolean isNotHaveSameNameParseResult = false;
		try {
			ParseResult parseResult = this.parseResultService.getParseResultBySign(MD5Utils.md5(title));
			isNotHaveSameNameParseResult = parseResult == null;
			// List<ParseResult> haveSameNameParseResults = this.parseResultService.getParseResultsByName(title);
			// isNotHaveSameNameParseResult = CollectionUtils.isEmpty(haveSameNameParseResults);
	 ...
	 * @param body
	 * @return
	 * @author:@haipenge haipenge@gmail.com 2014年9月20日
	 */
	 ...
	 * @param content
	 * @return
	 * @author:@haipenge haipenge@gmail.com 2015年1月30日
	 */
	 ...
	 * 
	 * @todo
	 * @author:@haipenge haipenge@gmail.com 2015年2月4日
	 */
	 ...
	 * @param content
	 * @return
	 * @author:@haipenge haipenge@gmail.com 2014年12月30日
	 */
	protected boolean isStore(String name, String content) {
		...
		// Rule 1:标题大于3个字符,内容长度超过50个字符
		if (StringUtils.isNotEmpty(name) && StringUtils.isNotEmpty(contentWords) && name.length() >= 3 && contentWords.length() > 30) {
			res = true;
		}
		return res;
	}
				...
				for (Map<String, String> map : matches) {
					...
					if (StringUtils.isNotEmpty(imgSrc)) {
						...
						if (StringUtils.isNotEmpty(imgSrc) && StringUtils.isNotEmpty(suffix)) {
							...
							content = content.replaceAll(img, "@@FaceYe-Img-" + id + "-@@");
						}
						// 生成图片的存储路径
						// String imgPath = "";
