	...
	public void filter(Document document, CrawlResult crawlResult, String content) {
		...
		if (StringUtils.isNotEmpty(content)) {
				...
				if (matcherConfig.getType().intValue() == 2) {
					try {
						...
						if (CollectionUtils.isNotEmpty(matchers)) {
							...
							categoryName=matcherConfig.getCategoryName();
							categoryAlias=matcherConfig.getCategoryAlias();
			...
			if (StringUtils.isNotEmpty(title)) {
				title=title.replaceAll("-美食菜谱--中华美食网", "");
				document.setTitle(title);
			}
			if(StringUtils.isNotEmpty(categoryName)){
				document.setCategoryName(categoryName);
			}
			if(StringUtils.isNotEmpty(categoryAlias)){
				document.setCategoryAlias(categoryAlias);
			}
		}
	}
